...
Les valeurs de résistances définies précédemment peuvent être pondérées par la pente \cite{2015_monkey}. Pour que la pente puisse être calculée, il faut tout d'abord charger un MNT à partir du menu Données | Charger un MNT. 
...
Le partitionnement est accessible depuis le menu contextuel d'un graphe (clic droit sur le nom d'un graphe). La première fenêtre demande de renseigner les paramètres $\alpha$ et $\beta$. Le paramètre $\alpha$ est déterminé à partir d'une distance $d$ et d'une probabilité $p$ (cf. \nameref{param_weight}). Pour définir $\alpha$ à 0, il faut mettre $p$ à 1. 
...
Après avoir défini une capacité différente de la surface de la tache, il peut être intéressant de supprimer les taches dont la capacité est trop faible. L'entrée "Supprimer des taches" du menu Fichier, permet de créer un nouveau projet identique au projet actuel en supprimant les taches ayant une capacité inférieure à un seuil donné.
