...
La deuxième fenêtre concerne l’importation de la carte du paysage. Celle-ci doit être un fichier de type raster (*.tif, *.asc, *.rst) dans lequel la valeur de chaque pixel correspond à une catégorie (occupation du sol ou autre type de classification).
...
Graphab permet de calculer les corridors représentant, pour une distance maximale donnée $d_{max}$, l'espace qui peut être traversé entre 2 taches d'habitat \textit{ie.} l'espace représentant l'ensemble des chemins possibles reliant 2 taches et ayant une distance inférieure à la distance $d_{max}$.
...
Le résultat s'affiche dans une nouvelle couche vectorielle contenant pour chaque lien du jeu de liens sélectionné un polygone représentant son corridor. Les liens ayant une distance supérieure à $d_{max}$ n'auront pas de corridor.

L'affichage par défaut utilise la transparence permettant de faire apparaître les superpositions de corridors.
...
\subsubsection{Probabilité de Connectivité Flux}
\label{metric_FPC}
\begin{table}[H]
\begin{tabular}{|m{3.24cm}|m{4.4810004cm}m{7.924cm}|}
