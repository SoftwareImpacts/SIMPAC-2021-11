...
\begin{itemize}
	...
	\item \verb|d=val| : distance permettant, avec le paramètre p, de définir le coefficient $\alpha$ 
	\item \verb|p=val| : probabilité permettant, avec le paramètre d, de définir le coefficient $\alpha$ 
\end{itemize}
...
\begin{itemize}
	\item \verb|npatch| : nombre d’éléments successifs à supprimer
	\item \verb|global_metric_name| : acronyme de la métrique globale (PC, ...)
	\item \verb|param1=val| : paramètre(s) de la métrique, si elle en a. Les intervalles ne sont pas permis pour cette commande.
\end{itemize}
...
Le paramètre \verb|multi| permet de tester l'ajout de \textit{nbpatch} taches simultané, dans un voisinage de rayon \textit{size}*\textit{gridres}.
