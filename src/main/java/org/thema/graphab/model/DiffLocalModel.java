...
public class DiffLocalModel {
    ...
    public static HashMap<DefaultFeature, Double> diff(final Project project, Pointset exoData, String varName, GraphGenerator graph,
            ...
            final double dMax, List<DefaultFeature> removalZones, TaskMonitor monitor) throws IOException {
                ...
                for (String var : vars) {
                    if (var.startsWith("ext-")) {
                        try {
                            ...
                            Logger.getLogger(DiffLocalModel.class.getName()).log(Level.FINER, "Point " + f.getId() + " is outside of grid " + var, ex);
                            xVal[k] = Double.NaN;
                        }
                    } else {
                        double sum = 0;
        ...
        HashMap<String, LocalMetric> localIndices = new HashMap<>();
        ...
        HashMap<DefaultFeature, Double> sumProbas;
        try (CSVWriter writer = new CSVWriter(new FileWriter(new File(project.getDirectory(), "sumproba.csv")))) {
            sumProbas = new HashMap<>();
            ...
            for (DefaultFeature remZone : removalZones) {
                ...
                for (Feature f : exoData.getFeatures()) {
                    if (((Number)f.getAttribute(varName)).doubleValue() == 1) {
                        ...
                        HashMap<DefaultFeature, Path> patchDists = new HashMap<>();
                        if (multiAttach) {
                            patchDists = new HashMap<>(cache.get(f.getGeometry()));
